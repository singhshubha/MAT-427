...
We evaluate every model’s performance in the Cross-Validation section using a number of repeated k-fold CV. For this, we divide the data into 3 folds and repeat it times consecutively. We measure the models’ accuracy and test them in the same way using Logistic Regression, Random Forest and XGBoost.

```{r}
set.seed(1234)
census_folds <- vfold_cv(census_train, v = 3, repeats = 3, strata = income)
...
IN this section, we are collecting the results from before and comparing. we make a nice table and arrange them in descending order. We observe that Random Forest has the best accuracy of about 86.5%.
...
Showing all the predicted values for the best model based on accuracy.

```{r}
# Predicting based on 0/1 levels.
rf_predictions <- predict(rf_fit_final, new_data = census_test, type = "class")
...
The final deliverable for the main project. Here the the column name have been sliced so the whole csv file contains only the prediction from the Random Forest Model.

```{r}
# Extract predictions as a single column tibble
submission <- tibble(income = rf_predictions$.pred_class)

# Write to CSV with no column name
write_csv(submission, "rf_predictions_submission.csv", col_names = FALSE)

```

## Conclusion

Overall, we built a complete machine learning process to predict if an individual’s income exceeds \$50,000 per year using U.S. Census data. When the data had been cleaned, and modelling done, we tested the performance of Logistic Regression, Random Forest and XGBoost side by side. The overall accuracy of Random Forest was around 86.5%. The test dataset was analyzed using the final model and the output was saved in a CSV file to submit. Altogether, the project showed that ensemble methods are effective in classification and in this project as well.
